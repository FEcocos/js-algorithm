 ...
 * 입력예제:
 * budget = 28
 * gifts = [[6, 6], [2, 2], [4, 3], [4, 5], [10, 3]]
 *
 * 출력예제:
 * 4
 *
 * 설명:
 * (2, 2), (4, 3), (4, 5)와 (10, 3)을 할인받아 (5, 3)에 사면
 * 비용이 4+7+9+8=28입니다.
 */

export function solution(budget, gifts) {
  let answer = 0;
  let n = gifts.length;
  gifts.sort((a, b) => {
  ...
  for(let i = 0; i < n; i++) {
    let money = budget - (gifts[i][0] / 2 + gifts[i][1]);
    let cnt = 1;
    for(let j = 0; j < n; j++) {
      if(j !== i && (gifts[j][0] + gifts[j][1] > money)) break;
      if(j !== i && (gifts[j][0] + gifts[j][1] <= money)) {
